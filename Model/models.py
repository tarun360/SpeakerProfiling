...
class UpstreamTransformer(nn.Module):
    def __init__(self, upstream_model='wav2vec2',num_layers=6, feature_dim=768, unfreeze_last_conv_layers=False):
        ...
        self.upstream.model.encoder.layers = self.upstream.model.encoder.layers[0:5]
        
        for param in self.upstream.model.encoder.layers.parameters():
            param.requires_grad = True
        
        for param in self.upstream.mode.feature_extractor():
            param.requires_grad = True
    ...
    def forward(self, x, x_len):
        ...
        x = self.upstream(x)['hidden_state_4']
        output_1 = self.transformer_encoder_1(x)
        output_2 = self.transformer_encoder_2(x)
        output_3 = self.transformer_encoder_3(x)
        output_averaged_1 = torch.mean(output_1, dim=1)#output_1.flatten(start_dim=1)
        output_averaged_2 = torch.mean(output_2, dim=1)#output_2.flatten(start_dim=1)
        output_averaged_3 = torch.mean(output_3, dim=1)#output_3.flatten(start_dim=1)
