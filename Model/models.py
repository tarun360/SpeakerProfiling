...
class UpstreamTransformer(nn.Module):
    def __init__(self, upstream_model='wav2vec2',num_layers=6, feature_dim=768, unfreeze_last_conv_layers=False):
        ...
        self.transformer_encoder_2 = torch.nn.TransformerEncoder(torch.nn.TransformerEncoderLayer(d_model=self.mfcc_feature_dim, nhead=4, batch_first=True), num_layers=num_layers)
        
        self.final_feature_dim = 800
    ...
    def forward(self, x, mfcc_x, x_len):
        ...
        output_2 = self.transformer_encoder_2(mfcc_x.squeeze(1))
