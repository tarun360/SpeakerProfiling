...
class CompactBilinearPoolingFn(Function):
    ...
    def forward(ctx, h1, s1, h2, s2, output_size, x, y, force_cpu_scatter_add=False):
        ...
        re = torch.fft.irfft(torch.complex(re_prod, im_prod), dim=-1, n=output_size)
    ...
    def backward(ctx,grad_output):
        ...
        grad_re_prod = torch.real(grad_prod)
        grad_im_prod = torch.imag(grad_prod)
        ...
        grad_fx = torch.fft.irfft(torch.complex(grad_re_fx,grad_im_fx), dim=-1, n=ctx.output_size)
        ...
        grad_fy = torch.fft.irfft(torch.complex(grad_re_fy,grad_im_fy), dim=-1, n=ctx.output_size)
